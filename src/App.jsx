...
function FluxoCaixaView({ transactions, accounts, onToggleConferido, onDelete, canDelete }) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {accounts.map((a) => (
          <Card key={a.id} className="flex items-center justify-between">
            <div>
              <p className="font-semibold flex items-center gap-2">{a.name} {!a.active && <Pill tone="neutral">inativa</Pill>}</p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      ...
      setAccounts((accountRows || []).map((a) => ({ id: a.id, name: a.name, active: a.active, saldoInicial: Number(a.saldo_inicial) })));
    ...
    <div className="min-h-screen flex">
      <aside className="hidden md:flex flex-col w-60 shrink-0 p-4" style={{ background: "var(--navy)" }}>
        <div className="flex items-center gap-2 mb-8 px-1">
          <Landmark color="#F1E6C3" size={22} /><span className="fin-display text-white font-semibold text-lg">Caixa</span>
        </div>
      ...
      {navOpen && (
        <div className="fixed inset-0 z-40 md:hidden" style={{ background: "rgba(18,33,59,0.6)" }} onClick={() => setNavOpen(false)}>
          <aside className="absolute left-0 top-0 bottom-0 w-64 p-4" style={{ background: "var(--navy)" }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2 mb-8 px-1"><Landmark color="#F1E6C3" size={22} /><span className="fin-display text-white font-semibold text-lg">Caixa</span></div>
      ...
      <main className="flex-1 min-w-0">
        ...
        <div className="p-4 md:p-7 max-w-6xl mx-auto">
          ...
          {tab === "fluxo" && <FluxoCaixaView transactions={transactions} accounts={accounts} onToggleConferido={toggleConferido} onDelete={deleteTransaction} canDelete={role.canDelete} />}
